...
public class ViewNewHolder {
    ...
    public ViewNewHolder(Callback caller, ViewGroup parent, int layoutId, int position) {
     ...
     * @return
     */
    public static ViewNewHolder getViewHolder(Callback caller, View convertView, ViewGroup parent, int layoutId, int position) {
        boolean needCreateView = false;
        ViewNewHolder vh = null;
        if (convertView == null) {
            needCreateView = true;
        } else {
            vh = (ViewNewHolder) convertView.getTag();
        }
        if (vh != null && (vh.mLayoutId != layoutId)) {
            needCreateView = true;
        }
        if (needCreateView) {
            return new ViewNewHolder(caller, parent, layoutId, position);
        }
        return (ViewNewHolder) convertView.getTag();
    }
    ...
    public void setImageForNet2(int viewId, String imgUrl, DisplayImageOptions options) {
        ImageView iv = getView(viewId);
